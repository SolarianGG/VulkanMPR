...
void Engine::immediate_submit(const std::function<void(VkCommandBuffer)> &function)
{
    ...
    const auto cmdSubmitInfo = utils::command_buffer_submit_info(m_immCommandBuffer);
                                      ...
                                      const VmaMemoryUsage memoryUsage)
{
    ...
    vmaCreateBuffer(m_allocator, &bufferCreateInfo, &allocCreateInfo, &alloc.buffer, &alloc.allocation,
                                    ...
                                    const bool mipMapped)
{
    ...
    if (mipMapped)
    {
        imageCreateInfo.mipLevels = utils::calculate_mip_levels(VkExtent2D{extent.width, extent.height});
    }
    ...
    VkImageViewCreateInfo imageViewCreateInfo = utils::image_view_create_info(format, image.image, aspectFlags);
                                    ...
                                    const VkImageUsageFlags imageUsage, const bool mipMapped)
{
    ...
    {
        throw std::runtime_error(std::format("Unsupported image format for now: {}", string_VkFormat(format)));
    }
    const auto bufferSize = extent.width * extent.height * extent.depth * imagePixelSize;
    ...
    auto *bufferData = static_cast<char *>(stagingBuffer.allocationInfo.pMappedData);
    ...
    immediate_submit([&](const VkCommandBuffer cmd) {
        ...
        barrierBuilder.add_image_barrier(
            image.transition({.stageMask = VK_PIPELINE_STAGE_2_TRANSFER_BIT,
        ...
        if (mipMapped)
        {
            utils::generate_mipmaps(cmd, image.image, VkExtent2D{extent.width, extent.height});
        }
        else
        {
...
void Engine::ensure_position_capacity(std::size_t additionalCount)
{
    ...
    const AllocatedBuffer newBuffer =
        create_buffer(newSize,
    ...
    if (m_globalPositionCount > 0)
    {
        immediate_submit([&](VkCommandBuffer cmd) {
            ...
            vkCmdCopyBuffer(cmd, m_globalPositionBuffer.buffer, newBuffer.buffer, 1, &copyRegion);
        });
...
void Engine::ensure_attributes_capacity(std::size_t additionalCount)
{
    ...
    if (m_globalAttributesCount > 0)
    {
        immediate_submit([&](VkCommandBuffer cmd) {
            ...
            vkCmdCopyBuffer(cmd, m_globalAttributesBuffer.buffer, newBuffer.buffer, 1, &copyRegion);
        });
...
void Engine::ensure_index_capacity(std::size_t additionalCount)
{
    ...
    const AllocatedBuffer newBuffer =
        create_buffer(newSize,
                      VK_BUFFER_USAGE_2_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_2_TRANSFER_DST_BIT |
                          VK_BUFFER_USAGE_2_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_2_SHADER_DEVICE_ADDRESS_BIT |
                          VK_BUFFER_USAGE_2_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR,
                      VMA_MEMORY_USAGE_GPU_ONLY);
    ...
    if (m_globalIndexCount > 0)
    {
        immediate_submit([&](VkCommandBuffer cmd) {
            ...
            vkCmdCopyBuffer(cmd, m_globalIndexBuffer.buffer, newBuffer.buffer, 1, &copyRegion);
        });
    ...
    mp::debug::set_object_name(m_device, VK_OBJECT_TYPE_BUFFER, reinterpret_cast<uint64_t>(m_globalIndexBuffer.buffer),
                               "Global Index Buffer");
    m_globalIndexBufferDeviceAddress = newBuffer.get_buffer_device_address(m_device);
